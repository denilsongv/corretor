...
ARQUIVO_DADOS = "leads_imobiliaria.json"
ARQUIVO_MENSAGENS = "mensagens_personalizadas.json"
ARQUIVO_AGENDA = "agenda_compromissos.json"
...
def carregar_compromissos():
    """Carrega compromissos do arquivo"""
    try:
...
def conectar_google_sheets():
    ...
    try:
        ...
        # Usa o arquivo credentials.json que está na mesma pasta
...
def carregar_leads_google():
    """Carrega leads do Google Sheets"""
    client = conectar_google_sheets()
    if not client:
        return []
    try:
        planilha = client.open("Sistema Imobiliario")
        aba = planilha.worksheet("leads")
        dados = aba.get_all_records()
        leads = []
        for i, row in enumerate(dados):
            # Pular cabeçalho se necessário (se a primeira linha for o cabeçalho)
...
def salvar_leads_google(leads):
    """Salva leads no Google Sheets"""
    client = conectar_google_sheets()
    if not client:
        st.warning("⚠️ Google Sheets não conectado. Verifique as credenciais.")
        return False
    ...
    try:
        ...
        try:
            ...
            planilha = client.create("Sistema Imobiliario")
            st.info("📊 Planilha 'Sistema Imobiliario' criada no Google Drive!")
        
        # Selecionar ou criar aba
        try:
            aba = planilha.worksheet("leads")
        except:
            aba = planilha.add_worksheet(title="leads", rows="1000", cols="20")
...
def gerar_mensagem_ia(lead, temperatura):
    """Gera mensagem automática da IA com suporte ao {link}"""
    ...
    link = lead.get('link_imovel', '#')  # Default to '#' if no link is provided
...
Quer dar uma olhada? Me avisa aí! 😊
...
Surgiram novidades no mercado! Que tal dar uma conferida? 😄
...
def formatar_telefone(telefone, link=False):
    """Formata telefone para link ou texto simples"""
    ...
    if link:
        return f"https://wa.me/{telefone_formatado}"  # Retorna link do WhatsApp
    return telefone_formatado

def gerar_mensagem_personalizada(lead, mensagem):
    """Substitui variáveis na mensagem personalizada com foco em {link}"""
    try:
        # Obter valores de nome e link
        nome = lead.get('nome', 'cliente').split()[0]
        link = lead.get('link_imovel', '').strip()

        # Garantir que o link tenha um valor padrão se estiver vazio
        if not link:
            link = 'https://example.com'  # Substitua pelo link padrão desejado
        ...
        # Log para depuração
        print(f"Valores substituídos: nome={nome}, link={link}")
...
def analisar_metricas(leads):
    ...
    origem_count = {}
    for lead in leads:
        origem = lead.get('origem', 'outros')
        origem_count[origem] = origem_count.get(origem, 0) + 1
    
    valor_count = {}
    for lead in leads:
        valor = lead.get('valor_imovel', 'N/A')
        valor_count[valor] = valor_count.get(valor, 0) + 1
    ...
    return {
        ...
        'origem_count': origem_count,
        'valor_count': valor_count
    }
...
def main():
    ...
    # Carregar dados (tentar do Google Sheets primeiro)
    if 'leads' not in st.session_state:
        leads_google = carregar_leads_google()
        if leads_google:
            st.session_state.leads = leads_google
        else:
    ...
    with st.sidebar:
        ...
        with col_salvar:
            if st.button("💾 Salvar", type="primary", use_container_width=True):
                if nome and telefone:
                    ...
                    salvar_leads(st.session_state.leads)
                    salvar_leads_google(st.session_state.leads)
                    st.rerun()
                else:
        ...
        st.markdown("---")
        st.markdown("### 💾 Backup")
        if st.button("📥 Exportar Backup", use_container_width=True):
            backup_nome = f"backup_leads_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
            shutil.copy(ARQUIVO_DADOS, backup_nome)
            st.success(f"✅ Backup salvo: {backup_nome}")
        
        uploaded_file = st.file_uploader("📤 Restaurar", type=['json'], key="restore")
        ...
        if st.button("🔄 Sincronizar com Google Sheets", use_container_width=True):
            if salvar_leads_google(st.session_state.leads):
                st.success("✅ Dados sincronizados com o Google Sheets!")
            else:
                st.error("❌ Erro ao sincronizar. Verifique suas credenciais.")
    ...
    with tab1:
        ...
        if st.session_state.leads:
            ...
            with col_f1:
                filtro_perfil = st.selectbox("Perfil", ["Todos"] + list(PERFIS.keys()), 
            ...
            if leads_filtrados:
                ...
                if metricas and lead_data['perfil'] in metricas['conversao_por_perfil']:
                    ...
                    st.info(f"🤖 **IA Recomenda:** Este perfil tem {taxa:.0f}% de conversão! Use as mensagens personalizadas.")
                ...
                with st.expander("📞 Ver informações completas"):
                    ...
                    st.markdown(f"**Link:** {lead_data.get('link_imovel', '-')}")
    ...
    with tab2:
        ...
        if metricas and metricas['total_leads'] > 0:
            # Gráfico de conversão por perfil
            st.markdown("### 🎯 Taxa de Conversão por Perfil")
            perfis = list(metricas['conversao_por_perfil'].keys())
            taxas = list(metricas['conversao_por_perfil'].values())
            
            df_graf = pd.DataFrame({
                'Perfil': [PERFIS[p]['titulo'] for p in perfis],
                'Conversão (%)': taxas
            })
            
            fig = px.bar(df_graf, x='Perfil', y='Conversão (%)', 
            ...
            if metricas['origem_count']:
                df_origem = pd.DataFrame({
                    'Origem': list(metricas['origem_count'].keys()),
                    'Quantidade': list(metricas['origem_count'].values())
                })
                fig2 = px.pie(df_origem, values='Quantidade', names='Origem', title="Distribuição por Origem")
                st.plotly_chart(fig2, use_container_width=True)
            
            # Gráfico de valores
            st.markdown("### 💰 Top Valores de Imóveis")
            if metricas['valor_count']:
                top_valores = sorted(metricas['valor_count'].items(), key=lambda x: x[1], reverse=True)[:10]
                df_valor = pd.DataFrame({
                    'Valor': [v[0] for v in top_valores],
                    'Quantidade': [v[1] for v in top_valores]
                })
                fig3 = px.bar(df_valor, x='Valor', y='Quantidade', title="Top 10 Valores de Imóveis")
    ...
    with tab3:
        ...
        with sub_tab1:
            ...
            st.markdown("Use as variáveis: `{nome}`, `{codigo}`, `{valor}`")
            ...
            mensagem_nova = st.text_area(
                "Escreva sua mensagem (use {nome}, {codigo}, {valor})",
                height=200,
                placeholder="Exemplo:\nOlá {nome}! 🏠✨\n\nSobre o imóvel {codigo} no valor de {valor}, tenho uma novidade..."
            )
            ...
            with col_criar1:
                if st.button("💾 Salvar Mensagem", type="primary", use_container_width=True):
                    if titulo and mensagem_nova:
                        ...
                        salvar_mensagens(st.session_state.mensagens_personalizadas)
            ...
            with col_criar2:
                ...
                **Exemplo:** "Olá {nome}! O imóvel {codigo} no valor de {valor}..."
                """)
                ...
                with st.container():
                    ...
                    with col3:
                        if st.button(f"✅ Concluir", key=f"concluir_{comp['id']}", use_container_width=True):
                            ...
                            salvar_compromissos(st.session_state.compromissos)
