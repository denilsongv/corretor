...
def carregar_campos_do_lead(lead):
    st.session_state["nome_lead"] = lead.get("nome", "")
    st.session_state["telefone_lead"] = lead.get("telefone", "")
    st.session_state["perfil_lead"] = lead.get("perfil", list(PERFIS.keys())[0])
    st.session_state["codigo_imovel_lead"] = lead.get("codigo_imovel", "")
    st.session_state["link_imovel_lead"] = lead.get("link_imovel", "")
    st.session_state["valor_imovel_lead"] = lead.get("valor_imovel", VALORES_IMOVEL[0])
    st.session_state["origem_lead"] = lead.get("origem", ORIGENS[0])
    st.session_state["status_lead"] = lead.get("status", STATUS_LISTA[0])
    st.session_state["observacoes_lead"] = lead.get("observacoes", "")
        ...
        if st.session_state.leads:
            ...
            if leads_filtrados:
                ...
                with col_btn3:
                    ...
                    st.markdown(
                        f'<a href="{link_whats}" target="_blank">'
                        f'<button style="width:100%;padding:0.5rem;border:none;border-radius:0.5rem;cursor:pointer;">💚 Abrir WhatsApp</button>'
                        f'</a>',
        ...
        with sub_tab2:
            ...
            if st.session_state.mensagens_personalizadas:
                ...
                if "editando_msg" in st.session_state:
                    ...
                    nova_categoria = st.selectbox("Categoria", categorias_msg, index=categorias_msg.index(msg_edit["categoria"]))
